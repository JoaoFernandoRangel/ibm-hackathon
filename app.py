...
if st.session_state.logged_in:
    ...
    if option == "Smart Clinical Summary":
        ...
        with c1:
            ...
            save_fb = st.checkbox("Save to Firebase (if configured)", value=False, key="smart_savefb")
        ...
        with center:
            if st.button("Generate clinical summary (from patient's JSON records)", key="smart_generate_button"):
                    ...
                    try:
                        if backend == "watsonx":
                            if not apikey:
                                ...
                                st.stop()
                            an_run = DiaryAnalyzer(backend="watsonx", watsonx_api_key=apikey, watsonx_project_id=project_id)
                        else:
                            an_run = DiaryAnalyzer(backend="local")

                        # prefer a dedicated method if available, else call generate_summary_from_text
                        if hasattr(an_run, "generate_summary_from_text"):
                            summary = an_run.generate_summary_from_text(merged, max_new_tokens=1024, tone=model_tone, custom_prompt=custom_prompt)
                        else:
                            summary = an_run.summarize_case(save_to_file=False, save_to_firebase=False, page_names=None, tone=model_tone, custom_prompt=custom_prompt, patient_data=None)
                    except Exception as e:
                ...
                if summary:
                    ...
                    if save_fb:
                        try:
                            save_week_summary({"email": patient_email, "resumo_clinico": summary})
                            st.success("Summary saved to Firebase!")
                        except Exception as e:
                            st.warning(f"Error saving summary to Firebase: {e}")
    ...
    if option == "Intelligent Clinical Summary":
        ...
        with c1:
            ...
            save_fb = st.checkbox("Save to Firebase (if configured)", value=False, key="intel_savefb")
            ...
            try:
                an_for_list = DiaryAnalyzer(backend="local")  # backend here doesn't affect list_patients()
                patients = an_for_list.list_patients() or {}
            except Exception as e:
                st.error(f"Failed to list patients: {e}")
                patients = {}
                ...
                if not patient_data:
                    ...
                    with st.spinner("Generating summary from patient form data (may take a while)..."):
                        try:
                            ...
                            summary = an.summarize_case(save_to_file=True, save_to_firebase=save_fb, page_names=None, tone=model_tone, custom_prompt=st.session_state.get("intel_custom_prompt",""), patient_data=patient_data)
